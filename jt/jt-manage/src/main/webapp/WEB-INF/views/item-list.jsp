...
<script>
    ...
    function getSelectionsIds(){
    	...
    	/*EasyUi提供的表格数据的方法  getSelections属性 
    	数组:保存选中数据 [item,item,item,item]
    	*/
    	var sels = itemList.datagrid("getSelections");
    	var ids = [];	//JS对象的数组. http协议只能传递字符串机字节码
    	for(var i in sels){
        	//循环遍历选中数据,之后动态获取item的Id值,保存到数组中.
    		ids.push(sels[i].id);
    	}
    	//将数组拼接成串 1,2,3,4,5
    	ids = ids.join(",");
    ...
    var toolbar = [{
        		...
        		onLoad :function(){
        			...
        			$.getJSON('/item/query/item/desc/'+data.id,function(_data){
        				/* if(result.status == 200){
        					var itemDesc = result.data;
        					var desc = itemDesc.itemDesc
        					itemEditEditor.html(desc);
        				} */
        				if(_data.status == 200){
        ...
        handler:function(){
        	...
        	$.messager.confirm('确认','确定下架ID为 '+ids+' 的商品吗？',function(r){
        	    if (r){
        	    	var params = {"ids":ids};  //{ids:100,101,102}
        	    	//发起下架请求.
